...
type CarouselProps<T> = { title: string; items: T[]; renderItem: (item: T, index: number) => ReactNode; measureItem?: (item: T, index: number) => ReactNode; ariaLabel?: string; id?: string; emptyText?: string; circular?: boolean }
...
export function Carousel<T>({ title, items, renderItem, measureItem, ariaLabel, id, emptyText, circular = false }: CarouselProps<T>) {
