...
export function LibraryCalendarPage({ schoolIdOverride, showSchoolSelector = true }: {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {effectiveSchoolId && <div className="flex flex-col items-start gap-3">
        <p aria-live="polite" className="text-sm text-slate-600 dark:text-slate-300">{isManager ? 'Você pode gerenciar os dias não úteis.' : 'Modo de consulta: somente leitura.'}</p>
        <div className={singleMonthMode ? 'flex w-full justify-center' : 'flex'}><div className="flex flex-col items-center gap-1" aria-label="Navegação do ano"><span className="text-sm font-semibold leading-tight text-slate-600 dark:text-slate-300">Ano</span><div className="flex items-center gap-1"><Button variant="secondary" size="sm" className="!p-2" onClick={() => setYear((value) => value - 1)} aria-label="Ano anterior"><ChevronLeft aria-hidden="true" /></Button><span className="min-w-[5ch] text-center text-lg font-bold leading-tight tabular-nums" aria-live="polite">{year}</span><Button variant="secondary" size="sm" className="!p-2" onClick={() => setYear((value) => value + 1)} aria-label="Próximo ano"><ChevronRight aria-hidden="true" /></Button></div></div></div>
      </div>}
